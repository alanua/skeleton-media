...
The source-owned script installs only the fixed package list, verifies the root device is not overlay/removable/USB-backed, validates only the four reviewed first-boot guard unit names before any package or config mutation, configures LightDM autologin for the configured media desktop user, prepares a blank Openbox desktop, mpv, Chromium policy, and enables/starts `ssh`, `lightdm`, and `avahi-daemon`. Openbox autostart does not launch PipeWire, pipewire-pulse, or WirePlumber; those user units are managed only through the live `/run/user/1000/bus` session with bounded `runuser` and `systemctl --user enable --now` argv. It never performs power, disk, bootloader, route, firewall, Tailscale, or private media mutations.
